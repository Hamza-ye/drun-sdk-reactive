...
mixin PathWalkingService<T extends FieldTemplate> {
  List<T> get flattenedFields;
  ...
  T? getFirstByName(String name) {
    final element = flattenedFields
        .firstOrNullWhere((field) => field.path!.endsWith('$name'));
    return element;
  }

  /// Get element by path
  T? getTemplateByPath(String path) {
    return flattenedFields.firstOrNullWhere((element) => element.path == path);
  }
  ...
  T? getScopedDependencyByName(String name, String currentPath) {
    ...
    final rootElements =
        flattenedFields.where((element) => element.path!.split('.').length == 1);
  ...
  T? getScopedDependencyByName2(String name, String currentPath) {
    ...
    final siblingElement = flattenedFields.firstOrNullWhere((element) =>
        element.name == name &&
        element.path!.startsWith(parentPath) &&
    ...
    return flattenedFields.firstOrNullWhere((element) =>
